...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in BertModelConcrete forward() function)
...
class BertSelfAttentionConcrete(BertSelfAttention):
    def __init__(self, config):
...
class BertAttentionConcrete(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = BertSelfAttentionConcrete(config)
...
class BertLayerConcrete(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = BertAttentionConcrete(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = BertAttentionConcrete(config)
...
class BertEncoderConcrete(nn.Module):
    ...
    def forward(
        ...
        return BaseModelOutputConcrete(
...
class BertModelConcrete(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.encoder = BertEncoderConcrete(config)
...
class BertForSequenceClassificationConcrete(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class BertForMultipleChoiceConcrete(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class BertForTokenClassificationConcrete(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class BertForQuestionAnsweringConcrete(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
