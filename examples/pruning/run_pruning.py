...
def compute_heads_importance(
    ...
    # Print/save matrices
    if not args.exact_pruning and save:
...
def gibbs_sampling_per_layer(    
    args, model, train_dataloader, val_dataloader, eval_dataloader, early_stop_step=2, K=4, deterministic=True, T=1
):
    if args.n_gpu > 1:
        n_layers, n_heads = model.module.config.num_hidden_layers, model.module.config.num_attention_heads
    else:
    ...
    while step < early_stop_step:
        ...
        if deterministic:
            current_heads_to_unmask = head_importance[layer].sort(descending = True)[1]

            current_heads_to_unmask = current_heads_to_unmask[:num_to_unmask]
            logger.info("Heads to unmask for layer %d: %s", layer, str(current_heads_to_unmask.tolist()))
        else:
            lambdas = np.float128(head_importance[layer].double().cpu().numpy())
            log_lambdas = np.log(lambdas) * (1/T)
            current_heads_to_unmask = log_sample_k_dpp(log_lambdas, num_to_unmask)
            logger.info("Heads to unmask for layer %d: %s", layer, str(current_heads_to_unmask))
        new_head_mask[layer] = 0.0
        new_head_mask[layer][current_heads_to_unmask] = 1.0
        ...
        if step % 2 == 1 and T > 0.001:
            T *= 0.1

    file_name = "gibbs_per_layer" + "_deterministic" if deterministic else ""
...
def gibbs_sampling(    
    ...
    while step < early_stop_step:
        ...
        if deterministic:
            ...
            lambdas = np.float128(head_importance.view(-1).double().cpu().numpy())
            log_lambdas = np.log(lambdas) * (1/T)
            current_heads_to_unmask = log_sample_k_dpp(log_lambdas, num_to_unmask)
            logger.info("Heads to unmask: %s", str(current_heads_to_unmask))
    ...
    file_name = "gibbs" + "_deterministic" if deterministic else ""
    save_results(args, val_scores, sparsities, all_head_masks, file_name)
    ...
    while True:
        ...
        if sparsities[i] == desired_sparsity:
            ...
            logger.info("Best head mask")
            print_2d_tensor(best_head_mask)
            np.save(os.path.join(args.output_dir, str(K) + '_best_mask.npy'), best_head_mask.cpu())
            break
        else:
            val_scores[i] = float("-inf")
...
def main():
    ...
    # scores, sparsities, all_head_masks = unmask_heads_f5(
    #     args, model, train_dataloader, eval_dataloader
    # )
    # logger.info("Area under curve: %.2f", auc(sparsities, scores))
    ...
    # if os.path.exists(os.path.join(args.output_dir, "all_S.npy")):
    #     all_S = np.load(os.path.join(args.output_dir, "all_S.npy"), allow_pickle=True)
    # else:
    #     all_S = compute_S(args, model, train_dataloader)
    # args.exact_pruning = True
    # scores, sparsities, all_head_masks = unmask_dpp_per_layer(
    #     args, model, train_dataloader, eval_dataloader
    # )
    # logger.info("Area under curve: %.2f", auc(sparsities, scores))

    # args.use_contexts = True
    # if os.path.exists(os.path.join(args.output_dir, "all_S_contexts.npy")):
    ...
    # head_importance = compute_heads_importance(args, model, train_dataloader)
    # scores, sparsities, all_head_masks = unmask_dpp(
    #     args, model, train_dataloader, eval_dataloader, all_S=all_S_contexts
    # )
    # logger.info("Area under curve: %.2f", auc(sparsities, scores))

    # args.use_contexts = False
    # scores, sparsities, all_head_masks = unmask_dpp(
    #     args, model, train_dataloader, eval_dataloader, all_S=all_S
    # )
    # logger.info("Area under curve: %.2f", auc(sparsities, scores))
    ...
    # mask_heads_random(args, model, eval_dataloader)
    # mask_layers(args, model, eval_dataloader)
    # plot_graph(args)
    for k in [3,4,5,6]:
        gibbs_sampling(args, model, train_dataloader, val_dataloader, eval_dataloader, early_stop_step=24, K=k, deterministic=False)

    for k in [3,4,5,6]:
        gibbs_sampling_per_layer(args, model, train_dataloader, val_dataloader, eval_dataloader, early_stop_step=24, K=k, deterministic=False)
